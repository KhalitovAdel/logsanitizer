...
describe('StringLineDecryption', () => {
    const instance = new StringLineDecryption();
    const testStrings = [
    ...
    it('should StringLineDecryption decrypt correct from string', async () => {
        testStrings.forEach((el) => {
            const result = instance.exec(el);
            expect(result.date).toBeInstanceOf(Date);
            expect(!!result.logLevel).toBeTruthy();
            expect(!!result.logLevel.length).toBeTruthy();
            expect(typeof result.jsonData === 'object').toBeTruthy();
        });
    });
});
